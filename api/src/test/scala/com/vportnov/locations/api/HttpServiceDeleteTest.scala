...
class HttpServiceDeleteTest extends AnyFlatSpec with GivenWhenThen:
  ...
  "DELETE /locations" should "return Ok if storage reports that nothing was deleted" in {
    ...
    And("uri has some valid location ids")
      val uri = apiUri("/locations?id=location123&id=location456")
  ...
  it should "fail with Bad Request if location ids list is empty" in {
    Given("service is connected to storage where no location exists ")
      val storage = new TestStorage[IO] {} 
      val service = new HttpService(storage, isSwaggerUIEnabled = false)
    
    And("uri does not have any location id")
      val uri = apiUri(s"/locations")
  ...
  it should "forward non empty id list to storage method" in {
    Given("uri has non empty list of location ids)")
      val expectedIds = List("location123", "location456")
      val uri = apiUri(s"/locations?id=${expectedIds.head}&id=${expectedIds.tail.head}")
  ...
  it should "fail with Bad Request if id is empty string" in {
    Given("uri has id which is empty string")

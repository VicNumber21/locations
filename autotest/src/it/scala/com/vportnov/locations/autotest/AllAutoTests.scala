...
// It mushed to implement custom framework (see SolutionContainers and AnyAutotestSpec) with some
// imperative logic.
...
// So to make it working all Autotest suites should be added to the list below and also
// marked by @DoNotDiscover in source file to prevent double run.

class AllAutoTests extends Suites(
  new SolutionTest
)
